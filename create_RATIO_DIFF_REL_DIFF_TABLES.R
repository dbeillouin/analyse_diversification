...
# split ratio, relative differences and differences ----------------------------------------------
    ...
    DIFF<-ES %>% filter(Metric_Output=="unit")
        ...
        # pour Angus, on transforme les régression en mean différence, en se basant sur la moyenne des valeurs observées
        DIFF  <-DIFF %>% mutate(ES= ifelse(ID=="Angus_2015" & Scenario_ID ==1,Reg_Intercept+ 3.46* Reg_Slope,ES))
        DIFF  <-DIFF %>% mutate(ES_lower_CI= ifelse(ID=="Angus_2015" & Scenario_ID ==1,(Reg_Intercept+ 3.46* Reg_Slope)-(Reg_SE/2*1.96),ES_lower_CI))
        DIFF  <-DIFF %>% mutate(ES_upper_CI= ifelse(ID=="Angus_2015" & Scenario_ID ==1,(Reg_Intercept+ 3.46* Reg_Slope)-(Reg_SE/2*1.96),ES_upper_CI))
