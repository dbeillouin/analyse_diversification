  ...
  # load Data
  source('~/Documents/analyse_diversification/analyse_diversification/3.Description_donnee.R', echo=TRUE)  
  ...
  model_rma_ID_NUM<-function(DAT) {
   ...
   # 1. Run the linear models (lm and rma)
    ...
    #2. Mixed models
      ...
      res$CAT<-paste0(DATA$Sub_Cat_Output,DATA$term)
      DATA<- res %>% filter(method=="lmer_ID")
      ggplot(data=DATA)+ 
        geom_point(aes(x=reorder(CAT ,estimate), y= estimate))+
        geom_errorbar(aes(x=CAT, ymin=conf.low, ymax=conf.high), width=.1) +
        theme_bw()+
        geom_hline(yintercept=0, linetype=2)+
        theme(axis.text.x = element_text(angle = 90, hjust = 1))+
      ...
      RATIO2$CAT<-paste0(RATIO2$Sub_Cat_Output,RATIO2$DIVERS)
      ESSAI<- RATIO2 %>% select(ES,Sub_Cat_Output, DIVERS)
  
      ESSAI2<-DATA %>%  select(estimate,Sub_Cat_Output,term,conf.low,conf.high)
      names(ESSAI2)[1]<-"ES"
      names(ESSAI2)[3]<-"DIVERS"
      ESSAI2$DIVERS<-substr(ESSAI2$DIVERS, 7,89)
     
      ESSAI3<-RATIO2 %>% group_by(Sub_Cat_Output, DIVERS)  %>%  count()
      
      ggplot(ESSAI, aes(x = Sub_Cat_Output, y = ES)) +
        geom_point(data = ESSAI2, color = "red")+
        facet_grid(~DIVERS, scales="free")+
